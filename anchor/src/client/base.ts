...
export class Base {
  ...
  constructor(cluster: ClusterNetwork, provider: anchor.Provider) {
    ...
    this.wallet = (this.provider as anchor.AnchorProvider).wallet as anchor.Wallet
  ...
  getErrorFromRPCResponse(rpcResponse: RpcResponseAndContext<SignatureResult | SimulatedTransactionResponse>) {
  ...
  async sendAndConfirm(tx: VersionedTransaction | Transaction, signer?: Keypair): Promise<TransactionSignature> {
    let txConnection: Connection
    if (this.cluster === ClusterNetwork.Mainnet) {
      const endpoint = process.env?.NEXT_PUBLIC_TX_RPC || process.env.TX_RPC || this.provider.connection.rpcEndpoint
    ...
    if (res.value.err) {
      ...
      const e = new BlsError(err, errTx?.meta?.err || undefined, errTx?.meta?.logMessages || [])
      e.customCode = code
      throw e
    }
    return txSig
...
export const isBrowser =
  ...
  process.env.ANCHOR_BROWSER || (typeof window !== 'undefined' && !window.process?.hasOwnProperty('type'))
